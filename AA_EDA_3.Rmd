...
trainrows2<-createDataPartition(acs_al$Airline, p = 0.7, list=FALSE) 
train.set.2<-acs_al[trainrows2,] 
test.set.2<-acs_al[-trainrows2,] 
print(nrow(train.set.2)) 
print(nrow(test.set.2)) 
...
```{r}
## anova analysis for Airport logit model
...
# find out how the raw features perform. 
Airline.logit.1 <- glm(Airline ~., data = train.set.2, family=binomial("logit")) 
print(Airline.logit.1)  
```


```{r}
## anova analysis for Airline logit model
anova(Airline.logit.1, test="Chisq")
```
+ Only Airport, Destination, Airfare, NoTransport, ModeTransport, Income are statistically significant.

```{r}
## Logistic regression 2 for Airline
Airline.logit.2 <- glm(Airline ~ Airport + Destination + NoTransport + ModeTransport + Income, data = train.set.2, family=binomial("logit")) 
print(Airline.logit.2)  
