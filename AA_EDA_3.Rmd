...
acs <- acs %>% group_by(Airport, Airline, Destination) %>% mutate(Airfare_avg = mean(Airfare,na.rm=T))
acs$Airfare[is.na(acs$Airfare)] <- acs$Airfare_avg[is.na(acs$Airfare)]
...
+ Observations on the data:
  ...
  + The majorities travel for leisure;
...
plots <- list()
for(nm in names(Filter(is.factor, acs_out))) {
    plots[[nm]] <- ggplot(data = acs_out) + geom_bar(aes_string(x=nm))
}
...
plots <- list()
for(nm in names(Filter(is.factor, acs_out))) {
    plots[[nm]] <- ggplot(data = acs_out,aes(fill = Airport))+ geom_bar(position = "dodge",aes_string(x=nm)) 
}

do.call(grid.arrange,plots[-c(1,2)])
```
...
plots <- list()
for(nm in names(Filter(is.factor, acs_out))) {
    plots[[nm]] <- ggplot(data = acs_out,aes(fill = Airline))+ geom_bar(position = "dodge",aes_string(x=nm)) 
}
